// Script simplificado para probar notificaciones de WhatsApp
...
async function testSimpleWhatsApp() {
  console.log('🧪 PROBANDO NOTIFICACIONES SIMPLES DE WHATSAPP');
  console.log('=============================================\n');
  ...
  try {
    ...
    const { data: producto, error: productoError } = await supabase
      ...
      .insert({
        ...
        descripcion: 'Producto para probar notificaciones de WhatsApp',
        precio: 15.99,
    ...
    const { data: cliente, error: clienteError } = await supabase
      .from('clientes')
      .insert({
        nombre: 'Cliente WhatsApp Test',
        email: 'cliente.whatsapp@prueba.com',
        telefono: '+34627388086',
        direccion: 'Calle WhatsApp 123'
      })
      .select()
      .single();
    ...
    if (producto && cliente) {
      ...
      const { data: encargo, error: encargoError } = await supabase
        ...
        .insert({
          ...
          notas: 'Encargo de prueba para WhatsApp'
        })
        .select()
        .single();
    ...
    if (notifError) {
      ...
      notifications.forEach((notif, index) => {
        ...
        if (notif.content) {
          console.log(`      Contenido: ${notif.content.substring(0, 50)}...`);
        }
      });
    }
